...
class GenDataIter:
    def __init__(self, samples, if_test_data=False, shuffle=None):
        ...
        main_data = self.__read_data__(samples)

        self.loader = DataLoader(
            dataset=GANDataset(main_data),
    ...
    def append_data(self, new_samples):
        new_data = self.__read_data__(new_samples)
        self.loader.dataset.data.extend(new_data)
...
class CompDisDataIter:
    def __init__(self, pos_samples, neg_samples, compile_result, shuffle=None):
        self.batch_size = cfg.batch_size
        self.max_seq_len = cfg.max_seq_len
        self.start_letter = cfg.start_letter
        self.shuffle = cfg.data_shuffle if not shuffle else shuffle
        ...
        self.loader = DataLoader(
            dataset=GANDataset(self.__read_data__(pos_samples, neg_samples, compile_result)),
            batch_size=self.batch_size,
            shuffle=self.shuffle,
            drop_last=True)

    def __read_data__(self, pos_samples, neg_samples, compile_result):
        """
        input: same as target, but start with start_letter.
        """
        inp, target = self.prepare(pos_samples, neg_samples, compile_result)
        all_data = [{'input': i, 'target': t} for (i, t) in zip(inp, target)]
        return all_data
    ...
    def prepare(self, pos_samples, neg_samples, compile_result, gpu=False):
        """Build inp and target"""
        inp = torch.cat((pos_samples, neg_samples), dim=0).long().detach()  # !!!need .detach()
        target = torch.ones(inp.size(0)).long()
        ...
        target[pos_samples.size(0):] = compile_result
        # shuffle
        perm = torch.randperm(inp.size(0))
        inp = inp[perm]
        target = target[perm]
