...
from utils.text_process import load_dict, write_tokens, tensor_to_tokens, write_tokens_to_files
...
class BasicInstructor:
    ...
    def train_gen_epoch(self, model, data_loader, criterion, optimizer):
        ...
        for i, data in tqdm(enumerate(data_loader)):
            ...
            #words = [self.idx2word_dict[str(key_value)] for key_value in inp[:, 3].cpu().numpy()]
            #print(words)    
            #versions = inp[:, 3]
            
            hidden = model.init_hidden(inp.shape[0])
            model.init_variable(inp.shape[0])
            pred = model.forward(inp, hidden, train_step=True, need_hidden=False)
            model.init_variable(inp.shape[0])
    ...
    def train_dis_epoch_with_compile(self, model, data_loader, criterion, optimizer, compile_result):
        total_loss = 0
        total_acc = 0
        total_num = 0
        for i, data in enumerate(data_loader):
            ...
            pred = model.forward(inp)
            target = [value + 1 if value == 1 else value for value in target]
    ...
    def _load(self, model_path):
        """Load model state dict and generator's samples"""
        # Load the model
